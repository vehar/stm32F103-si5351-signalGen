...
#include "i2c_wrapper.h" // Provides access to i2c_write_wrapper
#include <si5351.h>
...
void si5351_write(uint8_t reg, uint8_t value) { i2c_write_wrapper(reg, value); }
...
void si5351_SetupPLL(si5351PLL_t pll, si5351PLLConfig_t *conf)
{
    ...
    si5351_writeBulk(baseaddr, P1, P2, P3, 0);
...
int si5351_SetupOutput(uint8_t output, si5351PLL_t pllSource, si5351DriveStrength_t driveStrength,
...
void si5351_Calc(int32_t Fclk, si5351PLLConfig_t *pll_conf, si5351OutputConfig_t *out_conf)
{
    ...
    if (Fclk < 1000000)
    {
        // For frequencies in [8_000, 500_000] range we can use si5351_Calc(Fclk*64, ...) and
        // SI5351_R_DIV_64. In practice it's worth doing for any frequency below 1 MHz, since it
        // reduces the error.
        Fclk *= 64;
...
void si5351_write(uint8_t reg, uint8_t value) {
    while (HAL_I2C_IsDeviceReady(&I2C_HANDLE, (uint16_t)(SI5351_ADDRESS<<1), 3, HAL_MAX_DELAY) !=
HAL_OK) { }
